...
export async function getViewByPageName(
  ...
  const client = await pool.connect();
  try {
    // Note: In SQL, column names are often case-insensitive and returned as lowercase.
    // I've updated the property access to lowercase to be safe (e.g., viewData.pagename).
...
export async function addOrUpdateView(view: View): Promise<void> {
  ...
  try {
    ...
    // 2. Upsert the main view record
    // Note: We use lowercase for column names as PostgreSQL folds unquoted identifiers to lowercase.
    ...
    if (view.edges && view.edges.length > 0) {
      ...
      const edgePlaceholders = view.edges
        ...
        .join(", ");
      // Note: Edge 'id' is generated by the database, so we don't include it in the insert.
